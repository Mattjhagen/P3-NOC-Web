#!/bin/bash
# Setup Shaggoth AI Command Center on R410 via Tailscale
# DeepSeek-R1 671B Training Interface

set -e

R410_HOST="100.65.34.60"  # R410 Tailscale IP
R410_USER="matt"
VNC_PORT="5902"
WEB_PORT="6081"
...
echo "[1/7] Setting up SSH keys for R410 Tailscale..."
...
if ! ssh -o ConnectTimeout=10 -o BatchMode=yes ${R410_USER}@${R410_HOST} "echo 'Connected'" 2>/dev/null; then
    ...
    ssh-copy-id -o ConnectTimeout=15 ${R410_USER}@${R410_HOST} || {
        echo ""
        echo "ERROR: Failed to copy SSH key to R410"
        echo ""
        echo "Please verify:"
        echo "  1. R410 is online and connected to Tailscale"
        echo "  2. Tailscale IP is correct: ${R410_HOST}"
        echo "  3. SSH server is running on R410"
        echo "  4. You know the password for ${R410_USER}@${R410_HOST}"
        echo ""
        echo "Check R410 Tailscale status:"
        echo "  tailscale status | grep r410"
        echo ""
...
if ! ssh -o ConnectTimeout=10 ${R410_USER}@${R410_HOST} "echo 'Connected'" 2>/dev/null; then
    echo "ERROR: Still cannot connect to R410 via Tailscale"
    echo ""
    echo "Troubleshooting:"
    echo "  1. Check Tailscale is running: tailscale status"
    echo "  2. Check R410 is online in Tailscale network"
    echo "  3. Ping R410: ping ${R410_HOST}"
    exit 1
fi
...
# ============================================================
echo ""
echo "[4/7] Syncing Shaggoth AI trainer to R410 via Tailscale..."
...
sudo apt-get install -y htop -qq 2>/dev/null || true
...
# ============================================================
echo ""
echo "[6/7] Creating Tailscale connection scripts..."
...
echo "======================================================"
echo "  Shaggoth AI Command Center - Web Access (Tailscale)"
echo "======================================================"
echo ""
...
echo "======================================================"
echo "  Shaggoth AI Command Center - VNC (Tailscale)"
echo "======================================================"
echo ""
...
echo "Connecting to R410 Shaggoth AI via Tailscale..."
...
#!/bin/bash
# Sync Shaggoth AI changes to R410 via Tailscale

R410_HOST="${R410_HOST}"
R410_USER="${R410_USER}"

echo "Syncing Shaggoth AI to R410 via Tailscale..."
...
echo "✓ Sync complete via Tailscale"
EOF

chmod +x ~/P3_Official/P3-NOC-Web/sync-shaggoth-tailscale.sh

echo "  ✓ Sync script created: ./sync-shaggoth-tailscale.sh"
...
echo ""
echo "======================================================"
echo "  Shaggoth Command Center Ready on R410 (Tailscale)!"
echo "======================================================"
echo ""
...
echo ""
echo "======================================================"
echo "  Connection Methods (via Tailscale)"
echo "======================================================"
echo ""
echo "1. WEB BROWSER (Recommended)"
echo "   URL: http://${R410_HOST}:6081/vnc.html"
echo "   Script: ./shaggoth-web-tailscale.sh"
echo "   - Works anywhere on Tailscale network"
echo "   - No VPN config needed"
echo ""
echo "2. VNC CLIENT (Best Performance)"
echo "   Server: ${R410_HOST}:5902"
echo "   Password: shaggoth1"
echo "   Script: ./shaggoth-vnc-tailscale.sh"
echo ""
echo "3. SSH TERMINAL (Command Line)"
echo "   Script: ./shaggoth-ssh-tailscale.sh"
echo "   - Direct terminal access via Tailscale"
echo ""
echo "Shaggoth AI Trainer:"
echo "  Location: ~/AI/ on R410"
echo "  Interface: DeepSeek-R1 671B"
echo "  Platform: ~/Shaggoth-a1/ on R410"
echo "  Access: Via Tailscale from anywhere"
echo ""
echo "Sync updates:"
echo "  ./sync-shaggoth-tailscale.sh"
echo ""
echo "Monitor GPU:"
echo "  ssh ${R410_USER}@${R410_HOST} nvidia-smi"
echo ""
echo "Tailscale benefits:"
echo "  ✓ Secure encrypted connection"
echo "  ✓ Access from anywhere (not just local network)"
echo "  ✓ No port forwarding needed"
echo "  ✓ Works on phone/tablet with Tailscale app"
echo ""
